...
- Merch payments use Stripe Checkout and webhook-backed order status in test mode. Test checkout, paid webhooks, atomic paid-order inventory decrement, status-aware checkout return copy, buyer/seller/admin order history with subtotal, TTC fee, shipping, tax, discount, and total breakdowns, buyer-scoped receipt lookup with item lines, pending-order cleanup when checkout fails, safer local-order-before-Stripe-session creation, product refreshes after fulfillment/refund updates, shipping-required checkout behavior, admin order item visibility, and basic admin order controls are wired. Next payment work is production seller approval, Stripe Connect or manual payout policy, refund handling, receipt polish, fulfillment workflow, and payment-provider review.
