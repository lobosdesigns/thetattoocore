...
- Future booking flow: verified tattoo artists and studios should be able to connect an existing Google Calendar, show bookable availability, create appointment requests from DMs, and collect tattoo deposits through the same Stripe gateway before an appointment is confirmed. Keep this as a future integration so we can use a proper Google OAuth/calendar connector and Stripe-hosted deposit flow instead of storing calendar credentials or raw payment details ourselves.
...
- Column swipes should also reset the newly active column to the top and refresh against that column's newest ranked posts so a user lands at the current top of 4U, Gossip, Stuff, Gigs, Merch, or DM instead of staying mid-scroll from the previous column. Top reset is done for launch on swipe and tab/hash changes; ranking refresh remains part of the feed algorithm work.
...
- Search and Saved should mention Merch as limited/checkout-in-testing until products, seller approval, tax, shipping, refunds, fulfillment, payouts, and payment-provider rules are production-ready. Search now includes active, indexable public Merch previews, and Merch products can be saved from home/detail views and rendered in Saved.
...
- Merch payments use Stripe Checkout and webhook-backed order status in test mode. Test checkout, paid webhooks, atomic paid-order inventory decrement, status-aware checkout return copy, buyer/seller/admin order history with subtotal, TTC fee, shipping, tax, discount, and total breakdowns, buyer-scoped receipt lookup with item lines, receipt polish, pending-order cleanup when checkout fails, safer local-order-before-Stripe-session creation, product refreshes after fulfillment/refund updates, shipping-required checkout behavior, admin order item visibility with lifecycle dates, and basic admin order controls are wired. Next payment work is production seller approval, Stripe Connect or manual payout policy, deeper refund handling, fulfillment workflow, and payment-provider review.
- Stripe should become the site-wide payment gateway/POS layer for ads, Merch, and any future paid Stuff/booking/payment flows, so buyers, sellers, vendors, advertisers, and future appointment-deposit payers can pay or receive payouts through the platform without each area inventing separate payment logic. Started with Merch checkout as the first gateway-backed flow.
...
- TTC platform fees should be transparent and configurable by payment type. Start with a small 1-2% platform fee concept for Merch/vendor/artist sales and ads where legally and payment-provider appropriate, record it on every order/payment, and decide per flow whether the fee is buyer-paid, seller-deducted from payout, or split. Started with a transparent 2% test-mode Merch fee recorded as `platform_fee_cents` and displayed to buyers/admins. Taxes, refunds, disputes, card processing fees, and app-store rules need review before production rates are final.
...
- Ad payments should use the same Stripe gateway/POS foundation as Merch. Started with ad campaign payment fields for unpaid/checkout/paid/refunded state, prepaid amount, TTC platform fee, Stripe checkout session, Stripe payment intent, paid/refunded timestamps, advertiser Stripe Checkout for prepaid daily budget, duplicate checkout-start guards, webhook paid/failed/expired/full-refund updates, admin payment visibility, paid-or-waived delivery/click guards, and an admin guard that prevents unpaid ads from being activated unless payment is waived.
