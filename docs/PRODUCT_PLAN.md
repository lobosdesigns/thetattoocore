...
- Ad payments should use the same Stripe gateway/POS foundation as Merch. Started with ad campaign payment fields for unpaid/checkout/paid/refunded state, prepaid amount, TTC platform fee, Stripe checkout session, Stripe payment intent, paid/refunded timestamps, advertiser Stripe Checkout for prepaid daily budget, webhook status updates, admin payment visibility, and an admin guard that prevents unpaid ads from being activated unless payment is waived.
