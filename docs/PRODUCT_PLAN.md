...
- Add user-selectable appearance settings for Light, Dark, and System mode. Started for launch with instant local switching plus profile-stored preference for signed-in users; theme-token migration has begun on appearance controls, main column tabs, language cue, account/profile surfaces, DM shells, DM inbox/thread controls, notification list surfaces, Saved, Search, auth screens, 4U/Gossip detail pages, Stuff/Gigs detail pages, public policy/support pages, follower/following lists, public profile surfaces, composer modal controls, report controls, media upload controls, media lightbox, word/emoji field controls, profile/license form controls, account settings shell/cards, install/share controls, notification icon, sensitive-content gate, and admin mail settings so both modes can become polished instead of just inverted.
