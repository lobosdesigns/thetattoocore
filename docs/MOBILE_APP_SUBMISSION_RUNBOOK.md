...
- Google Play submissions or updates on or after August 31, 2026 must target Android 16 / API 36. The checked-in and active closed-test release `1.0.3 (4)` uses that baseline. Any later replacement must increment the version code above `4`, rebuild and sign a fresh upload bundle, and rerun wrapper plus real-device QA before selecting a Google Play track.
...
- Google Play target API: new apps and updates submitted on or after August 31,
  ...
  `1.0.3 (4)` is the current API 36 baseline; any replacement must use a version
  code above `4`.
...
- Confirm `npm.cmd run smoke:mobile` at 390px and `npm.cmd run smoke:mobile:narrow` at 320x568 cover Support, Help, Child Safety Standards, Privacy, and Terms routes plus the Search/Saved, Booking, Ads, Merch, Verification, and privacy/safety Help Center guides, including the Merch storefront and Merch Help guide, before store screenshots or wrapper QA.
...
- Run `npm.cmd run smoke:app-links` after every deploy that changes app-link routing or private association identifiers. It should either confirm valid association JSON or the expected fail-closed unavailable response before final identifiers are configured.
...
6. Upload to Google Play closed/internal testing and Apple TestFlight before any public review submission. Google Play Closed testing - Alpha now serves API 36 release `1.0.3 (4)` to eligible testers; iOS build `1.0 (4)` is available to `TTC Internal Testers`, while App Store version `1.0` continues review with build `1.0 (3)`.
   ...
   - July 23, 2026 Google Play status: notification-icon update `1.0.3 (4)` completed automated checks and review, was published to Closed testing - Alpha, and was installed from Google Play on the authorized Android 16 review phone. Exact-build/API 36, verified App Links, production-link launch, retained-session landing, and system-bar framing checks passed; build 4 is now the only controlled Android alert-QA allowlist.
