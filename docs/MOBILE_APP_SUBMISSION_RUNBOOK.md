...
- Run `npm.cmd run verify:native-predevice` before reviewer screenshots, console-copy validation, or store-console handoff when a release-candidate Android device is not authorized yet. It checks environment boundaries, private native config exclusions, private handoff-template validation, readiness docs, store metadata, Android-profile mobile routes, and iOS-profile mobile routes without counting as real-device evidence.
- Run `npm.cmd run verify:native-release` before final native handoff. It checks environment boundaries, private native config exclusions, private handoff-template validation, and readiness docs first, intentionally fails until the Android probe sees an authorized device with the TTC package installed, then continues through store, Android-profile mobile, and iOS-profile mobile smoke checks.
