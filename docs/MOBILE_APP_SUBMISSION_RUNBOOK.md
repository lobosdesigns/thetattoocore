...
- Run `npm.cmd run verify:native-release` before final native handoff. It checks environment boundaries and private native config exclusions first, intentionally fails until the Android probe sees an authorized device with the TTC package installed, then continues through native, store, Android-profile mobile, and iOS-profile mobile smoke checks.
