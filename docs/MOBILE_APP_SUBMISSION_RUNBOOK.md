...
- The checked-in Android candidate is `1.0.3 (4)`. It adds the dedicated monochrome TTC notification status icon. The signed API 36 bundle was accepted into the existing Alpha track and the single change was sent for review after confirming zero supported-device loss; it remains staged until Google Play serves it, it is installed from Alpha, and the authorized phone passes exact-build QA. Keep the completed `1.0.2 (3)` evidence and alert allowlist unchanged until then.
