...
- The checked-in Android candidate is `1.0.3 (4)`. It adds the dedicated monochrome TTC notification status icon and remains staged until a signed bundle is accepted, served by Google Play, installed from the Alpha track, and verified on the authorized phone. Keep the completed `1.0.2 (3)` evidence and alert allowlist unchanged until then.
