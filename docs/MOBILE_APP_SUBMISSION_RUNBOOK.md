...
- Run `npm.cmd run verify:release-evidence -- --release-candidate <current-production-version>` against the ignored private handoff before final store submission. The current production version is mandatory, and the gate fails if the handoff's web deploy row is stale. This strict gate also checks the exact Android Alpha `1.0.2 (3)`, App Review `1.0 (3)`, TestFlight `1.0 (4)`, reviewer access, store fields and media, tester install, real-device QA, two-user messaging, native alert delivery/routing, and legal signoff. It reports only repo-safe requirement names and is expected to fail until every private evidence row is complete.
