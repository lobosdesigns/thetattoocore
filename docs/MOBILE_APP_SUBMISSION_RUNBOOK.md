...
- The checked-in Android candidate is `1.0.3 (4)`. It adds the dedicated monochrome TTC notification status icon. The signed API 36 bundle was accepted into the existing Alpha track, automated checks completed successfully, and the single change is in review after confirming zero supported-device loss; it remains staged until Google Play serves it, it is installed from Alpha, and the authorized phone passes exact-build QA. Keep the completed `1.0.2 (3)` evidence and alert allowlist unchanged until then.
...
- Google Play submissions or updates on or after August 31, 2026 must target Android 16 / API 36. The checked-in Android candidate `1.0.3 (4)` and active closed-test release `1.0.2 (3)` use that baseline. Any later replacement must increment the version code above `4`, rebuild and sign a fresh upload bundle, and rerun wrapper plus real-device QA before selecting a Google Play track.
