...
- Run `npm.cmd run verify:store-release` before final screenshot upload validation so production environment boundaries, store metadata, PWA install assets, readiness docs, public routes, Android-profile mobile routes, and iOS-profile mobile routes are checked together.
