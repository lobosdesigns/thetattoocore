...
- Run `npm.cmd run verify:payment-go-live` as the separate final evidence gate before real-money checkout is promoted. It reads `private-release-handoff/release-handoff-template.md` by default and exits with named blockers when the file, required payment rows, dashboard review dates, results, or private proof locations are missing, `pending`, or `blocked`. Every required Payments blocker and Payment Dashboard row must name a non-placeholder private proof filename or location; `fixture-only` is accepted only with explicit `--test-fixture` mode. The gate also rejects payment-flow evidence from a stale release candidate; each flow's Release candidate cell must match the current Git commit, with a 7-40 character commit prefix accepted.
- When the reviewed release candidate is a deployed commit other than the current checkout, run `npm.cmd run verify:payment-go-live -- --release-candidate <commit-sha>`. Use `--evidence <private-path>` only for an ignored private handoff copy. Do not point the production command at committed fixtures.
- `npm.cmd run test:payment-go-live-gate` proves the strict parser's passing path with a sanitized, explicitly marked fixture. Fixture mode is restricted to `scripts/fixtures`, announces that it is not release evidence, and cannot be used by the default production command.
