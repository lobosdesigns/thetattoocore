...
- July 22, 2026 dashboard inspection still shows Connect setup and account/go-live verification work in progress; its next setup item is `Create a test connected account`. The production Admin payment preflight shows explicit mode `Needs review`, server payment key mode `Test`, webhook signing `Ready`, and checkout blocked until the expected mode is readable and matched. Live-money cutover remains blocked until those setup rows, webhook mode/event proof, Admin reconciliation, penny-test proof, refund/dispute procedure, payout gate, and native checkout policy review are recorded in the private handoff.
