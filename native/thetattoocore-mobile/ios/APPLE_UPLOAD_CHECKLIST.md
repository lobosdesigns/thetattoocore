...
- Confirm version: `1.0`.
- Confirm the archive source matches checked-in iOS source candidate `1.0 (5)`. Source identity does not establish the App Store Connect or TestFlight selection.
- The native smoke guard derives only the checked-in Xcode version/build. Separately verify the exact current App Review and TestFlight identities read-only before choosing any upload or selection action.
- Treat build `5` as the source candidate only. Do not call it uploaded, selected, available, or installed without separate console/device evidence and explicit authorization.
...
- Keep messaging auto-initialization and production delivery off until explicit member opt-in, token registration, notification delivery, tap routing, and opt-out pass on build `5`.
...
Do not begin an archive, upload, TestFlight assignment, or App Review selection from this checklist until separate authorization follows the read-only identity verification above.

## Historical Apple Baseline - July 18-24, 2026 (Non-Operative)

The dated baseline recorded an Organizer upload of iOS `1.0 (3)` and later internal notification-capability evidence for `1.0 (4)`. Preserve that private console and device evidence as history only. It does not establish any current App Review, TestFlight, or installed-device identity and cannot authorize replacement, resubmission, or upload work.
