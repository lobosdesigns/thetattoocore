...
checks.push({
  ...
  ok:
    packageJson.includes(
      '"verify:payment-release": "npm run smoke:env && npm run smoke:payments && npm run smoke:security && npm run smoke:docs && npm run smoke:public"',
    ) &&
    paymentReadiness.includes("npm.cmd run verify:payment-release") &&
    paymentReadiness.includes("environment mode checks, payment flow guards, security headers, readiness docs, and public checkout/status routes"),
});
